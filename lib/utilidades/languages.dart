// List of Languages
// 0 - English
// 1 - Portugues
// 2 - Spanish
// 3 - Marroquí
// 4 - Frances (not used yet)

// Listing of messages
final lang = {
  'Check out the CCSE Fácil app to prepare for your citizenship exam!': [
    'Check out the CCSE Fácil app to prepare for your citizenship exam!',
    'Confira o aplicativo CCSE Fácil para se preparar para o seu exame de cidadania!',
    '¡Descarga la aplicación CCSE Fácil para prepararte para tu examen de ciudadanía!',
    'تحقق من تطبيق CCSE Fácil للتحضير لامتحان المواطنة الخاص بك!',
    'Découvrez l\'application CCSE Fácil pour vous préparer à votre examen de citoyenneté !'
  ],
  'Next Exam Dates': [
    'Next Exam dates',
    'Próximas datas de exame',
    'Próximas fechas de examen',
    'تواريخ الامتحانات القادمة',
    'Prochaines dates d\'examen'
  ],
  'I saw this ad in the CCSE Fácil app and am insterested in learning more about it:\n\n':
      [
    'I saw this ad in the CCSE Fácil app and am insterested in learning more about it:\n\n',
    'Vi este anúncio no aplicativo CCSE Fácil e estou interessado em saber mais sobre ele:\n\n',
    'Vi este anuncio en la aplicación CCSE Fácil y estoy interesado en aprender más sobre él:\n\n',
    'رأيت هذا الإعلان في تطبيق CCSE Fácil وأنا مهتم بمعرفة المزيد عنه:\n\n',
    'J\'ai vu cette annonce dans l\'application CCSE Fácil et je suis intéressé à en savoir plus à ce sujet :\n\n'
  ],
  'Source link not available': [
    'Source link not available',
    'Link da fonte não disponível',
    'Enlace de la fuente no disponible',
    'رابط المصدر غير متوفر',
    'Lien source non disponible'
  ],
  'Source': ['Source', 'Fonte', 'Fuente', 'المصدر', 'Source'],
  'Source not available': [
    'Source not available',
    'Fonte não disponível',
    'Fuente no disponible',
    'المصدر غير متوفر',
    'Source non disponible'
  ],
  'Rank our app on the Store': [
    'Rank our app on the Store',
    'Avalie nosso aplicativo na Loja',
    'Califica nuestra aplicación en la Tienda',
    'قم بتقييم تطبيقنا على المتجر',
    'Évaluez notre application sur le Store'
  ],
  'Follow us on Instagram': [
    'Follow us on Instagram',
    'Siga-nos no Instagram',
    'Síguenos en Instagram',
    'تابعنا على إنستغرام',
    'Suivez-nous sur Instagram'
  ],
  'Share this app with your friends': [
    'Share this app with your friends',
    'Compartilhe este aplicativo com seus amigos',
    'Comparte esta aplicación con tus amigos',
    'شارك هذا التطبيق مع أصدقائك',
    'Partagez cette application avec vos amis'
  ],
  'Please accept the terms and select your country to continue.': [
    'Please accept the terms and select your country to continue.',
    'Por favor, aceite os termos e selecione seu país para continuar.',
    'Por favor, acepte los términos y seleccione su país para continuar.',
    'يرجى قبول الشروط وتحديد بلدك للمتابعة.',
    'Veuillez accepter les conditions et sélectionner votre pays pour continuer.'
  ],
  'We recommend you to practice at least 150 questions before taking the simulated exam. This will help you become more familiar with the question formats and improve your chances of success.':
      [
    'We recommend you to practice at least 150 questions before taking the simulated exam. This will help you become more familiar with the question formats and improve your chances of success.',
    'Recomendamos que você pratique pelo menos 150 perguntas antes de fazer o exame simulado. Isso ajudará você a se familiarizar mais com os formatos das perguntas e a melhorar suas chances de sucesso.',
    'Recomendamos que practiques al menos 150 preguntas antes de realizar el examen simulado. Esto te ayudará a familiarizarte más con los formatos de las preguntas y a mejorar tus posibilidades de éxito.',
    'نوصي بأن تمارس ما لا يقل عن 150 سؤالًا قبل إجراء الامتحان التجريبي. سيساعدك ذلك على التعرف بشكل أفضل على تنسيقات الأسئلة وتحسين فرصك في النجاح.',
    'Nous vous recommandons de pratiquer au moins 150 questions avant de passer l\'examen simulé. Cela vous aidera à vous familiariser davantage avec les formats des questions et à améliorer vos chances de réussite.'
  ],
  'You have not practiced any questions yet. Start practicing to see your progress here!':
      [
    'You have not practiced any questions yet. Start practicing to see your progress here!',
    'Você ainda não praticou nenhuma pergunta. Comece a praticar para ver seu progresso aqui!',
    'Aún no has practicado ninguna pregunta. ¡Comienza a practicar para ver tu progreso aquí!',
    'لم تمارس أي أسئلة بعد. ابدأ في الممارسة لرؤية تقدمك هنا!',
    'Vous n\'avez encore pratiqué aucune question. Commencez à pratiquer pour voir vos progrès ici !'
  ],
  'No Questions Practiced': [
    'No Questions Practiced',
    'Nenhuma Pergunta Praticada',
    'Ninguna Pregunta Practicada',
    'لم يتم ممارسة أي أسئلة',
    'Aucune question pratiquée'
  ],
  'You Have practiced': [
    'You Have practiced',
    'Você Praticou',
    'Has practicado',
    'لقد مارست',
    'Vous avez pratiqué'
  ],
  'of': ['of', 'de', 'de', 'من', 'de'],
  'Questions': ['Questions', 'Perguntas', 'Preguntas', 'أسئلة', 'Questions'],
  'and responded correctly to': [
    'and responded correctly to',
    'e respondeu corretamente a',
    'y respondiste correctamente a',
    'وأجبت بشكل صحيح على',
    'et avez répondu correctement à'
  ],
  'of them': ['of them', 'delas', 'de ellas', 'منها', 'd\'entre eux'],
  'Follow us on Social Media': [
    'Follow us on social media',
    'Siga-nos nas redes sociais',
    'Síguenos en las redes sociales',
    'تابعنا على وسائل التواصل الاجتماعي',
    'Suivez-nous sur les réseaux sociaux'
  ],
  'Deactivate Developer Mode': [
    'Deactivate Developer Mode',
    'Desativar Modo de Desenvolvedor',
    'Desactivar Modo de Desarrollador',
    'إلغاء تنشيط وضع المطور',
    'Désactiver le mode développeur'
  ],
  'Activate Developer Mode': [
    'Activate Developer Mode',
    'Ativar Modo de Desenvolvedor',
    'Activar Modo de Desarrollador',
    'تفعيل وضع المطور',
    'Activer le mode développeur'
  ],
  'Dear User': [
    'Dear User',
    'Caro Usuário',
    'Estimado Usuario',
    'عزيزي المستخدم',
    'Cher utilisateur'
  ],
  'Continue': ['Continue', 'Continuar', 'Continuar', 'استمر', 'Continuer'],
  'trckMsg': [
    'We care about your privacy and data security. We keep this app free by showing ads. Can we continue to use your data to tailor ads for you? You can change your choice anytime in the app settings. Thank you for Supporting us!',
    'Nós nos preocupamos com sua privacidade e segurança de dados. Mantemos este aplicativo gratuito exibindo anúncios. Podemos continuar a usar seus dados para personalizar os anúncios para você? Você pode alterar sua escolha a qualquer momento nas configurações do aplicativo. Obrigado por nos apoiar!',
    'Nos preocupamos por su privacidad y la seguridad de sus datos. Mantenemos esta aplicación gratuita mostrando anuncios. ¿Podemos continuar usando sus datos para personalizar los anuncios para usted? Puede cambiar su elección en cualquier momento en la configuración de la aplicación. ¡Gracias por apoyarnos!',
    'نحن نهتم بخصوصيتك وأمان بياناتك. نحافظ على هذا التطبيق مجانيًا من خلال عرض الإعلانات. هل يمكننا الاستمرار في استخدام بياناتك لتخصيص الإعلانات لك؟ يمكنك تغيير اختيارك في أي وقت في إعدادات التطبيق. شكرًا لدعمنا!',
    'Nous nous soucions de votre vie privée et de la sécurité de vos données. Nous maintenons cette application gratuite en affichant des publicités. Pouvons-nous continuer à utiliser vos données pour personnaliser les publicités pour vous ? Vous pouvez changer votre choix à tout moment dans les paramètres de l\'application. Merci de nous soutenir !'
  ],
  'Progress': ['Progress', 'Progresso', 'Progreso', 'التقدم', 'Progrès'],
  'Click to learn more about how you have to take the exam.': [
    'Click to learn more about how you have to take the exam.',
    'Clique para saber mais sobre como você deve fazer o exame.',
    'Haga clic para obtener más información sobre cómo debe realizar el examen.',
    'انقر لمعرفة المزيد حول كيفية إجراء الامتحان.',
    'Cliquez pour en savoir plus sur la façon dont vous devez passer l\'examen.'
  ],
  'You can also choose to look at interesting facts that can help you learn about Spanish Government, Economics, Society and Culture.':
      [
    'You can also choose to look at interesting facts that can help you learn about Spanish Government, Economics, Society and Culture.',
    'Você também pode escolher olhar para fatos interessantes que podem ajudá-lo a aprender sobre o governo, a economia, a sociedade e a cultura da Espanha.',
    'También puedes optar por ver datos interesantes que pueden ayudarte a aprender sobre el gobierno, la economía, la sociedad y la cultura de España.',
    'يمكنك أيضًا اختيار الاطلاع على حقائق مثيرة للاهتمام يمكن أن تساعدك في التعرف على الحكومة الإسبانية والاقتصاد والمجتمع والثقافة.',
    'Vous pouvez également choisir de regarder des faits intéressants qui peuvent vous aider à en apprendre davantage sur le gouvernement, l\'économie, la société et la culture espagnols.'
  ],
  'Study Cards': [
    'Study Cards',
    'Cartões de Estudo',
    'Tarjetas de Estudio',
    'بطاقات الدراسة',
    'Cartes d\'étude'
  ],
  'Practice by Category': [
    'Practice by Category',
    'Praticar por Tema',
    'Practicar por Tema',
    'الممارسة حسب الموضوع',
    'Pratiquer par catégorie'
  ],
  'Practice Cards': [
    'Practice Cards',
    'Cartões de Prática',
    'Tarjetas de Práctica',
    'بطاقات الممارسة',
    'Cartes de pratique'
  ],
  'Review': ['Review', 'Revisar', 'Revisar', 'مراجعة', 'Revue'],
  'Quick': ['Quick', 'Rápido', 'Rápido', 'سريع', 'Rapide'],
  'Start Exam': [
    'Start Exam',
    'Iniciar Exame',
    'Iniciar Examen',
    'بدء الامتحان',
    'Commencer l\'examen'
  ],
  'Study by Category': [
    'Study by Category',
    'Estudar por Tema',
    'Estudiar por Tema',
    'الدراسة حسب الموضوع',
    'Étudier par catégorie'
  ],
  'CCSE - Simulated Exam': [
    'CCSE - Simulated Exam',
    'CCSE - Simulado',
    'CCSE - Examen Simulado',
    'CCSE - امتحان محاكاة',
    'CCSE - Examen Simulé'
  ],
  'CCSE Exam Simulation': [
    'CCSE Exam Simulation',
    'Simulado do Exame CCSE',
    'Simulación del Examen CCSE',
    'محاكاة امتحان CCSE',
    'Simulation de l\'examen CCSE'
  ],
  'Portuguese': [
    'Portuguese',
    'Português',
    'Portugués',
    'البرتغالية',
    'Portugais'
  ],
  'Spanish': ['Spanish', 'Espanhol', 'Español', 'الإسبانية', 'Espagnol'],
  'Moroccan Arabic': [
    'Moroccan Arabic',
    'Árabe Marroquino',
    'Árabe Marroquí',
    'العربية المغربية',
    'Arabe Marocain'
  ],
  'English': ['English', 'Inglês', 'Inglés', 'الإنجليزية', 'Anglais'],
  'French': ['French', 'Francês', 'Francés', 'الفرنسية', 'Français'],
  'Language for translation': [
    'Language for translation',
    'Idioma para tradução',
    'Idioma para traducción',
    'اللغة للترجمة',
    'Langue pour la traduction'
  ],
  'CCSE Manual': [
    'CCSE Manual',
    'Manual CCSE',
    'Manual CCSE',
    'دليل CCSE',
    'Manuel CCSE'
  ],
  'Try the CCSE Exam': [
    'Try the CCSE Exam',
    'Tente o Exame CCSE',
    'Intenta el Examen CCSE',
    'جرب امتحان CCSE',
    'Essayez l\'examen CCSE'
  ],
  'Reset statistics': [
    'Reset Statistics',
    "Redefinir Estatísticas",
    "Restablecer Estadísticas",
    "إعادة تعيين الإحصائيات",
    "Réinitialiser les statistiques"
  ],
  'Re-start the statistics': [
    'Re-start the statistics',
    'Reiniciar Estatísticas',
    'Reiniciar Estadísticas',
    'إعادة تشغيل الإحصائيات',
    'Redémarrer les statistiques'
  ],
  "The CCSE exam includes 25 questions extracted from the 300 exercises of the CCSE learning manual. You will have 45 minutes to complete it, and needs to respond correctly to at least 15 questions to pass.":
      [
    "The CCSE exam includes 25 questions extracted from the 300 exercises of the CCSE learning manual. You will have 45 minutes to complete it, and needs to respond correctly to at least 15 questions to pass.",
    "O exame CCSE inclui 25 perguntas extraídas das 300 que fazem parte do manual de aprendizagem CCSE. Você terá 45 minutos para completá-lo e precisa responder corretamente a pelo menos 15 perguntas para ser aprovado.",
    "El examen CCSE incluye 25 preguntas extraídas de las 300 que hay en el manual de aprendizaje CCSE. Tendrá 45 minutos para completarlo y necesita responder correctamente al menos 15 preguntas para aprobar.",
    "يشمل امتحان CCSE 25 سؤالًا مستخرجًا من دليل التعلم CCSE. سيكون لديك 45 دقيقة لإكماله ، ويجب أن تجيب بشكل صحيح على 15 سؤالًا على الأقل للنجاح.",
    "L'examen CCSE comprend 25 questions extraites des 300 exercices du manuel d'apprentissage CCSE. Vous disposerez de 45 minutes pour le compléter et devrez répondre correctement à au moins 15 questions pour réussir."
  ],
  'You can chose how many questions to practice, and will have: Immediate validation of your response, knowledge cards to help you memorize, and translation to your language if you want.':
      [
    "You can chose how many questions to practice, and will have: Immediate validation of your response, knowledge cards to help you memorize, and translation to your language if you want.",
    "Você pode escolher quantas perguntas praticar e terá: Validação imediata de sua resposta, cartões de conhecimento para ajudá-lo a memorizar e tradução para seu idioma, se desejar.",
    "Puedes elegir cuántas preguntas practicar y tendrás: Validación inmediata de tu respuesta, tarjetas de conocimiento para ayudarte a memorizar y traducción a tu idioma si lo deseas.",
    "يمكنك اختيار عدد الأسئلة التي ترغب في ممارستها ، وستحصل على: التحقق الفوري من إجابتك ، وبطاقات المعرفة لمساعدتك على الحفظ ، والترجمة إلى لغتك إذا كنت ترغب في ذلك.",
    "Vous pouvez choisir combien de questions pratiquer, et vous aurez : Validation immédiate de votre réponse, cartes de connaissances pour vous aider à mémoriser, et traduction dans votre langue si vous le souhaitez."
  ],
  "Govern and Legislation": [
    "Govern and Legislation",
    "Governo e Legislação",
    "Gobierno y Legislación",
    "الحكومة والتشريع",
    "Gouvernement et législation"
  ],
  "Rights and Fundamental Duties": [
    "Rights and Fundamental Duties",
    "Direitos e Deveres Fundamentais",
    "Derechos y Deberes Fundamentales",
    "الحقوق والواجبات الأساسية",
    "Droits et devoirs fondamentaux"
  ],
  "Physical and Political Geography": [
    "Physical and Political Geography",
    "Geografia Física e Política",
    "Geografía Física y Política",
    "الجغرافيا الفيزيائية والسياسية",
    "Géographie physique et politique"
  ],
  "Culture and History of Spain": [
    "Culture and History of Spain",
    "Cultura e História da Espanha",
    "Cultura e Historia de España",
    "ثقافة وتاريخ إسبانيا",
    "Culture et histoire de l'Espagne"
  ],
  "Spanish Society": [
    "Spanish Society",
    "Sociedade Espanhola",
    "Sociedad Española",
    "المجتمع الإسباني",
    "Société espagnole"
  ],
  "Simulated Exam": [
    "Simulated Exam",
    "Exame Simulado",
    "Examen Simulado",
    "الامتحان التجريبي",
    "Examen simulé"
  ],
  'Exit Exam?': [
    'Exit Exam?',
    'Sair do Exame?',
    '¿Salir del Examen?',
    'هل تريد الخروج من الامتحان؟',
    'Voulez-vous quitter l\'examen ?'
  ],
  'Are you sure you want to exit the exam?': [
    'Are you sure you want to exit the exam?',
    'Tem certeza de que deseja sair do exame?',
    '¿Está seguro de que desea salir del examen?',
    'هل أنت متأكد أنك تريد الخروج من الامتحان؟',
    'Êtes-vous sûr de vouloir quitter l\'examen ?'
  ],
  'Yes': ['Yes', 'Sim', 'Sí', 'نعم', 'Oui'],
  'Cancel': ['Cancel', 'Cancelar', 'Cancelar', 'إلغاء', 'Annuler'],
  'questions from all categories.': [
    'questions from all categories.',
    'perguntas de todas as categorias.',
    'preguntas de todas las categorías.',
    'أسئلة من جميع الفئات.',
    'Questions de toutes les catégories.'
  ],
  'Practice': ['Practice', 'Pratique', 'Pratique', 'مارس', 'Pratiquer'],
  'You have finished your exam:': [
    'You have finished your exam:',
    'Você terminou seu exame:',
    'Has terminado tu examen:',
    'لقد أنهيت امتحانك:',
    'Vous avez terminé votre examen:'
  ],
  'PASSED': ['PASSED', 'APTO', 'APTO', 'مقبول', 'RÉUSSI'],
  'CONGRATULATIONS': [
    'CONGRATULATIONS',
    'PARABÉNS',
    'ENHORABUENA',
    'تهانينا',
    'FÉLICITATIONS'
  ],
  'PRACTICE MORE': [
    'PRACTICE MORE',
    'PRATIQUE MAIS',
    'PRATIQUE MÁS',
    'مارس أكثر',
    'PRATIQUEZ PLUS'
  ],
  'FAILED': ['FAILED', 'INAPTO', 'INAPTO', 'غير مقبول', 'ÉCHOUÉ'],
  'Take the test': [
    'Take the test',
    'Fazer o teste',
    'Hacer el examen',
    'قم بإجراء الاختبار',
    'Passer le test'
  ],
  'Time is up!': [
    'Time is up!',
    'O tempo acabou!',
    '¡Se acabó el tiempo!',
    'انتهى الوقت!',
    'Le temps est écoulé !'
  ],
  'Printed': [
    'was printed already',
    'voce respondeu já',
    'tu has respondido ya',
    'تم الطباعة بالفعل',
    'a été imprimé déjà'
  ],
  'Correct': [
    'was correct',
    'resposta correta',
    'respuesta correcta',
    'كان صحيحا',
    'était correct'
  ],
  'Success Rate': [
    'Success Rate',
    'Taxa de Sucesso',
    'Tasa de Éxito',
    'معدل النجاح',
    'Taux de réussite'
  ],
  'Accumulated Success Rate': [
    'Accumulated Success Rate',
    'Taxa de Sucesso Acumulada',
    'Tasa de Éxito Acumulada',
    'معدل النجاح المتراكم',
    'Taux de réussite accumulé'
  ],
  'Application is not connected to servers': [
    'Application is not connected to servers',
    'Aplicativo não está conectado aos servidores',
    'La aplicación no está conectada a los servidores',
    'التطبيق غير متصل بالخوادم',
    'L\'application n\'est pas connectée aux serveurs'
  ],
  'Connection to servers established': [
    'Connection to servers established',
    'Conexão com os servidores estabelecida',
    'Conexión con los servidores establecida',
    'تم إنشاء اتصال بالخوادم',
    'Connexion aux serveurs établie'
  ],
  'Total Questions': [
    'Total Questions',
    'Total de Perguntas',
    'Total de Preguntas',
    'إجمالي الأسئلة',
    'Total des questions'
  ],
  'Total Questions Answered': [
    'Total Questions Answered',
    'Total de Perguntas Respondidas',
    'Total de Preguntas Respondidas',
    'إجمالي الأسئلة التي تم الرد عليها',
    'Total des questions répondues'
  ],
  'Reset Statistics': [
    'Reset Statistics',
    'Reiniciar Estatísticas',
    'Reiniciar Estadísticas',
    'إعادة تعيين الإحصائيات'
  ],
  'Online Mode Activated': [
    'Online Mode Activated',
    'Modo Online Ativado',
    'Modo Online Activado',
    'تم تفعيل الوضع عبر الإنترنت',
    'Mode en ligne activé'
  ],
  'Offline Mode Activated': [
    'Offline Mode Activated',
    'Modo Offline Ativado',
    'Modo Offline Activado',
    'تم تفعيل الوضع غير المتصل بالإنترنت',
    'Mode hors ligne activé'
  ],
  'Select your country:': [
    'Select your country:',
    'Selecione seu país:',
    'Seleccione tu país:',
    'حدد بلدك:',
    'Sélectionnez votre pays:'
  ],
  'Your Country of Citizenship': [
    'Your Country of Citizenship',
    'Seu País de Cidadania',
    'Tu País de Ciudadanía',
    'بلدك الأصلي',
    'Votre pays de citoyenneté'
  ],
  'Select your language:': [
    'Select your language:',
    'Selecione seu idioma:',
    'Seleccione tu idioma:',
    'حدد لغتك:',
    'Sélectionnez votre langue:'
  ],
  'Available Languages': [
    'Available Languages',
    'Idiomas Disponíveis',
    'Idiomas Disponibles',
    'اللغات المتاحة',
    'Langues disponibles'
  ],
  'Your Language': [
    'Your Language',
    'Seu Idioma',
    'Tu Idioma',
    'لغتك',
    'Votre langue'
  ],
  'Search': ['Search', 'Pesquisar', 'Buscar', 'بحث', 'Recherche'],
  'Credits and Acknowledgments': [
    'Credits and Acknowledgments',
    'Créditos e Agradecimentos',
    'Créditos y Reconocimientos',
    'تقديرات وشكر',
    'Crédits et remerciements'
  ],
  'Terms of Use': [
    'Terms of use',
    'Termos de Uso',
    'Términos de Uso',
    'شروط الاستخدام',
    'Conditions d\'utilisation'
  ],
  'Privacy Policy': [
    'Privacy Policy',
    'Política de Privacidade',
    'Política de Privacidad',
    'سياسة الخصوصية',
    'Politique de confidentialité'
  ],
  'Accepted': ['Accepted', 'Aceito', 'Aceptado', 'مقبول', 'Accepté'],
  'Answered': [
    'Answered',
    'Respondido',
    'Respondido',
    'تم الرد عليه',
    'Répondu'
  ],
  'Answered Questions': [
    'Answered Questions',
    'Perguntas Respondidas',
    'Preguntas Respondidas',
    'الأسئلة التي تم الرد عليها',
    'Questions répondues'
  ],
  'Correct Answers': [
    'Correct Answers',
    'Respostas Corretas',
    'Respuestas Correctas',
    'الإجابات الصحيحة',
    'Réponses correctes'
  ],
  'Number of Questions': [
    'Number of Questions',
    'Número de Perguntas',
    'Número de Preguntas',
    'عدد الأسئلة',
    'Nombre de questions'
  ],
  'Wrongly': ['Wrongly', 'Errado', 'Incorrecto', 'خطأ', 'Mal'],
  'Incorrect': [
    'was incorrect',
    'resposta incorreta',
    'respuesta incorrecta',
    'كانت الإجابة غير صحيحة',
    'était incorrecte'
  ],
  'Correctly': [
    'Correctly',
    'Corretamente',
    'Correctamente',
    'صحيح',
    'Correctement'
  ],
  'Wrong': ['Wrong', 'Errado', 'Incorrecto', 'خطأ', 'Faux'],
  'Question': ['Question', 'Pergunta', 'Pregunta', 'سؤال', 'Question'],
  'times': ['times', 'vezes', 'veces', 'مرات', 'fois'],
  'problem loading': [
    'Problem loading data',
    'Problema ao carregar os dados',
    'Problema al cargar los datos',
    'مشكلة في تحميل البيانات',
    'Problème de chargement des données'
  ],
  'All': ['All', 'Completo', 'Completo', 'الكل', 'Tout'],
  'Only': ['Only', 'Somente', 'Solo', 'فقط', 'Seulement'],
  'Unanswered': [
    'Unanswered',
    'Não Respondidas',
    'No Respondidas',
    'غير مجابة',
    'Non répondues'
  ],
  'Wrong Answers': [
    'Wrong Answers',
    'Respostas Erradas',
    'Respuestas Incorrectas',
    'إجابات خاطئة',
    'Réponses incorrectes'
  ],
  'Training Trail': [
    'Training Trail',
    'Trilha de Treinamento',
    'Ruta de Entrenamiento',
    'مسار التدريب',
    'Sentier de formation'
  ],
  'Training Trail Description': [
    'We recommend that you train your knowledge in steps, that are ordered below. Select the step that you want to train at the moment.',
    'Recomendamos que você treine seu conhecimento em etapas, que estão ordenadas abaixo. Selecione a etapa que deseja treinar no momento.',
    'Recomendamos que entrenes tu conocimiento en pasos, que están ordenados a continuación. Selecciona el paso que deseas entrenar en este momento.',
    'نوصي بأن تقوم بتدريب معرفتك على مراحل، مرتبة أدناه. حدد المرحلة التي ترغب في التدريب عليها في الوقت الحالي.',
    'Nous vous recommandons de former vos connaissances par étapes, qui sont ordonnées ci-dessous. Sélectionnez l\'étape que vous souhaitez entraîner pour le moment.'
  ],
  'How you are progressing': [
    'How you are progressing',
    'Como você está progredindo',
    'Cómo estás progresando',
    'كيف تتقدم',
    'Comment progressez-vous'
  ],
  'This Training': [
    'This Training',
    'Este Treinamento',
    'Este Entrenamiento',
    'هذا التدريب',
    'Cette formation'
  ],
  'You have finished your training. These are your results': [
    'You have finished your training. These are your results',
    'Você terminou seu treinamento. Estes são os seus resultados',
    'Has terminado tu entrenamiento. Estos son tus resultados',
    'لقد أنهيت تدريبك. هذه هي نتائجك',
    'Vous avez terminé votre formation. Voici vos résultats'
  ],
  'Share your experience': [
    'Share your experience',
    'Compartilhe sua experiência',
    'Comparte tu experiencia',
    'شارك تجربتك',
    'Partagez votre expérience'
  ],
  'Help us improve the app': [
    'Help us improve the app',
    'Ajude-nos a melhorar o aplicativo',
    'Ayúdanos a mejorar la aplicación',
    'ساعدنا في تحسين التطبيق',
    'Aidez-nous à améliorer l\'application'
  ],
  'Restart': ['Restart', 'Reiniciar', 'Reiniciar', 'إعادة تشغيل', 'Redémarrer'],
  'Device ID': [
    'Device ID',
    'ID do Dispositivo',
    'ID del Dispositivo',
    'معرف الجهاز',
    'ID de l\'appareil'
  ],
  'Save': ['Save', 'Salvar', 'Guardar', 'حفظ', 'Enregistrer'],
  'Your Name': ['Your Name', 'Seu Nome', 'Tu Nombre', 'اسمك', 'Votre nom'],
  'Confirm Reset': [
    'Confirm Reset',
    'Confirmar Reinicialização',
    'Confirmar Reinicio',
    'تأكيد إعادة الضبط',
    'Confirmer la réinitialisation'
  ],
  'Are you sure you want to reset the statistics?': [
    'Are you sure you want to reset the statistics?',
    'Você tem certeza que deseja reiniciar as estatísticas?',
    '¿Estás seguro de que deseas reiniciar las estadísticas?',
    'هل أنت متأكد أنك تريد إعادة تعيين الإحصائيات؟',
    'Êtes-vous sûr de vouloir réinitialiser les statistiques ?'
  ],
  'Cred_Opening_message': [
    'We proudly acknowledge the collaboration of our valued partners—advocates of open technologies, Spanish education professionals, and legal experts committed to supporting the citizenship process. Their insight and dedication have been essential in shaping an app that is both accurate and accessible for everyone preparing for the CCSE exam.',
    'Reconocemos con orgullo la colaboración de nuestros valiosos socios: defensores de las tecnologías abiertas, profesionales de la educación española y expertos legales comprometidos con el apoyo al proceso de ciudadanía. Su conocimiento y dedicación han sido esenciales para dar forma a una aplicación que es tanto precisa como accesible para todos los que se preparan para el examen CCSE.',
    'Reconhecemos com orgulho a colaboração de nossos valiosos parceiros: defensores de tecnologias abertas, profissionais de educação espanhola e especialistas jurídicos comprometidos em apoiar o processo de cidadania. Seu conhecimento e dedicação foram essenciais para moldar um aplicativo que é preciso e acessível para todos que se preparam para o exame CCSE.',
    'نحن نعتز بالتعاون مع شركائنا الموقرين - المدافعين عن التقنيات المفتوحة ، والمهنيين في التعليم الإسباني ، والخبراء القانونيين الملتزمين بدعم عملية المواطنة. كانت رؤاهم وتفانيهم أساسيين في تشكيل تطبيق دقيق وسهل الوصول للجميع الذين يستعدون لامتحان CCSE.',
    'Nous reconnaissons avec fierté la collaboration de nos précieux partenaires - défenseurs des technologies ouvertes, professionnels de l\'éducation espagnole et experts juridiques engagés à soutenir le processus de citoyenneté. Leur perspicacité et leur dévouement ont été essentiels pour façonner une application à la fois précise et accessible à tous ceux qui se préparent à l\'examen CCSE.'
  ],
  'List of partners': [
    'List of partners',
    'Lista de parceiros',
    'Lista de socios',
    'قائمة الشركاء',
    'Liste des partenaires'
  ],
  'Rating saved successfully': [
    'Rating saved successfully',
    'Avaliação salva com sucesso',
    'Calificación guardada con éxito',
    'تم حفظ التقييم بنجاح',
    'Évaluation enregistrée avec succès'
  ],
  'Rating': ['Rating', 'Avaliação', 'Calificación', 'تقييم', 'Évaluation'],
  'Language of the App': [
    'Language of the App',
    'Idioma do App',
    'Idioma de la App',
    'لغة التطبيق',
    'Langue de l\'application'
  ],
  'msg_config': [
    'Configure your app to improve your experience',
    'Configure seu aplicativo para melhorar sua experiência',
    'Configura tu aplicación para mejorar tu experiencia',
    'قم بتكوين تطبيقك لتحسين تجربتك',
    'Configurez votre application pour améliorer votre expérience'
  ],
  'msg_terms': [
    'Accept the terms of use to use the app',
    'Aceite os termos de uso para usar o aplicativo',
    'Acepta los términos de uso para utilizar la aplicación',
    'اقبل شروط الاستخدام لاستخدام التطبيق',
    'Acceptez les conditions d\'utilisation pour utiliser l\'application'
  ],
  'CCSE Fácil news': [
    'CCSE Fácil news',
    'Notícias do CCSE Fácil',
    'Novedades de CCSE Fácil',
    'أخبار CCSE Fácil',
    'Nouvelles de CCSE Fácil'
  ],
  'If you have any questions, complaints or suggestions': [
    'If you have any questions, complaints or suggestions',
    'Se você tiver alguma dúvida, reclamação ou sugestão',
    'Si tienes alguna pregunta, queja o sugerencia',
    'إذا كانت لديك أي أسئلة أو شكاوى أو اقتراحات',
    'Si vous avez des questions, des plaintes ou des suggestions'
  ],
  'Configurations': [
    'Set up',
    'Configurações',
    'Configuraciones',
    'إعدادات',
    'Configurations'
  ],
  'Name, Country, Language': [
    'Name, Country, Language',
    'Nome, País, Idioma',
    'Nombre, País, Idioma',
    'الاسم، البلد، اللغة',
    'Nom, Pays, Langue'
  ],
  'Support': ['Support', 'Suporte', 'Soporte', 'الدعم', 'Support'],
  'In Progress': [
    'In Progress',
    'Em Progresso',
    'En Progreso',
    'قيد التقدم',
    'En cours'
  ],
  ' English': ['English', 'Inglês', 'Inglés', 'الإنجليزية', 'Anglais'],
  ' Portuguese': [
    'Portuguese',
    'Português',
    'Portugués',
    'البرتغالية',
    'Portugais'
  ],
  ' Spanish': ['Spanish', 'Espanhol', 'Español', 'الإسبانية', 'Espagnol'],
  ' Moroccan Arabic': [
    'Moroccan Arabic',
    'Árabe Marroquino',
    'Árabe Marroquí',
    'العربية المغربية',
    'Arabe Marocain'
  ]
};
